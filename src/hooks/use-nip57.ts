...
export const useNIP57 = ({ communityNpub, satsAmount, onPaymentSuccess, onPaymentError }: NIP57PaymentProps) => {
  const [communityProfile, setCommunityProfile] = useState<NDKUser | null>(null);
  ...
  const handleZap = async () => {
    ...
    if (!communityProfile) return handleError('Perfil de la comunidad no cargado aún, intenta de nuevo');
    ...
    const signer = typeof window !== 'undefined' && window.nostr ? new NDKNip07Signer() : null;
    ...
    try {
      ...
      if (!window.webln?.enable || !window.webln?.sendPayment) {
        return handleError('Tu navegador no tiene Alby u otra wallet LN compatible instalada para poder pagar');
      }
