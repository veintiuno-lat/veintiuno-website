...
const LnPayment: React.FC<LnPaymentProps> = ({ lightningAddress, npub }) => {
  const [step, setStep] = useState<
    "input" | "loading" | "waiting" | "success" | "error"
  >("input");
  ...
  const { handleZap, isLoading, signer } = useNIP57({
    communityNpub: npub || "",
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (Date.now() - startTime >= timeoutMs) {
        setErrorMessage(
          "El pago no se confirmó a tiempo. Cierra esta ventana y verifica tu wallet."
        );
        setStep("error");
        setVerifyUrl(null);
        return;
      ...
      try {
        const response = await fetch(getUrlWithBypass(verifyUrl), {
          cache: "no-store",
        });
        if (!response.ok) {
        ...
        setErrorMessage(
          "Hubo un problema al verificar el pago. Cierra esta ventana y revisa tu wallet."
        );
        setStep("error");
        setVerifyUrl(null);
        return;
            ...
            <div className='flex flex-col items-center justify-center p-4 gap-4'>
              ...
              <div className='w-full relative mt-4'>
                <textarea
                  readOnly
                  value={invoice}
                  className='w-full p-2 pr-10 border rounded-md bg-gray-100 text-xs text-gray-700 resize-none font-mono'
                  rows={4}
                />
                <Button
                  variant='ghost'
                ...
                {npub && signer && (
                  <>
                    ...
                    <Button
                      onClick={handleZap}
                      size='lg'
                      className='w-full bg-purple-600 hover:bg-purple-400 mt-4'
                      disabled={isLoading}
                    >
                      {isLoading && (
          ...
          {step === "success" && (
            <div className='flex flex-col items-center justify-center p-8 gap-4 text-center'>
              ...
              <p className='text-gray-600'>
                Gracias por tu donación de {amount} sats.
              </p>
              <DialogClose asChild>
